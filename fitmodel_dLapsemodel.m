function result = fitmodel_dLapsemodel(indata,parameters_to_split)
    ...
    betalabel_comm = {'\mu','\lambda','\alpha+','\alpha-','lapse'};
    inx_comm = [    1  1.1   0.8   0.8   0];
    lb_comm  = [ 0.01  0.5   0.3   0.3   0];
    ub_comm  = [   20    5   1.3   1.3  0.8];
    
    betalabel_rare = {'dMu','dLambda','dAlpha+','dAlpha-','dLapse'}; 
    inx_rare = zeros(1,length(betalabel_rare));
    lb_rare  = [-20 -3 -1 -1 -1];
    ub_rare  = [ 20  3  1  1  1];
    
    [splits,~] = ismember(betalabel_comm,parameters_to_split); % for the rare trial parameters that do not get fit, they are fixed to zero
    ...
    inx = [inx_comm inx_rare]; % base model only
    lb = [lb_comm lb_rare]; % base model only
    ub = [ub_comm ub_rare]; % base model only
    ...
    result.nosplit = true(1,length(betalabel_rare)); % do not split on any parameter
    ...
    try
        ...
        step1_params_basemodel  = b(result.paramsactive); 
...
function [loglike, utildiff, logodds, probchoice] = model_param(x, data)
    ...
    param_vecs{5} = repelem(data.lapse_comm,size(data.data,1))';
...
%     if data.nosplit==0 % if the parameter has a dParameter, we implement it here
%         param_vecs{7}(oddball) = x(7)+x(8); 
%     end
%     
    mu_vec = param_vecs{1};
    ...
    probchoice = (probchoice.*(1-lapse_vec))+ (lapse_vec./2);
