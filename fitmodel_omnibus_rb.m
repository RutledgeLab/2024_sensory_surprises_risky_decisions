function result = fitmodel_omnibus_rb(indata,parameters_to_split)
    
    % features 6 parameters: mu, lambda, alphagain, alphaloss,risky bias
    ...
    betalabel_comm = {'\mu','\lambda','\alpha+','\alpha-','risky bias'};
    inx_comm = [    1  1.1   0.8   0.8    0];
    lb_comm  = [ 0.01  0.5   0.3   0.3   -3];
    ub_comm  = [   20    5   1.3   1.3    3];
    
    betalabel_rare = {'{\Delta} Mu','{\Delta} Lambda','{\Delta} Alpha+','{\Delta} Alpha-','{\Delta} risky bias'}; 
...
function [loglike, utildiff, logodds, probchoice] = model_param(x, data)
    ...
    param_vecs{5} = repelem(data.bias_comm,size(data.data,1))';
    ...
    probchoice = (1 ./ (1+exp(-(logodds + bias_vec))));
